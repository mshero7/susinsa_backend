...
public class UserController {
	...
	public ResponseEntity<JSONResult> checkId(
			@ModelAttribute @Valid UserCheckIdDTO dto, BindingResult result) {
		
		if(result.hasErrors()) 
			return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(JSONResult.fail(result.getAllErrors().get(0).getDefaultMessage()));
		
		Boolean exist = userService.existId(dto);
